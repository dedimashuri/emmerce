...
class User_Cart extends Component {
    ...
    return this.props.dataUser.cart.map((val, index) => {
      return (
        <tr key={index} style={{ fontWeight: '500', fontSize: '13px' }}>
          ...
          <td width="180px" style={{ verticalAlign: "middle", textAlign: 'center' }}>{val.nama}</td>
          <td width="80px" style={{ verticalAlign: "middle", textAlign: 'center' }}>
            <img src={val.foto} alt={val.name} width="80px" height="80px" />
          </td>
          <td width="120px" style={{ verticalAlign: "middle", textAlign: 'center' }}>{currencyFormatter(val.harga)}</td>
          ...
          <td width="150px" style={{ verticalAlign: "middle", textAlign: 'center' }}>{currencyFormatter(val.harga * val.qty)}</td>
  ...
  onCheckoutClick = async () => {
    ...
    await axios.post(`${API_URL}/transaksi`, data).then((res) => {
      console.log(res)
    })
...
export default connect(MaptstatetoProps, { CartAction })(User_Cart);